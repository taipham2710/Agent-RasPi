...
class Config:
    ...
    # MQTT settings
    MQTT_BROKER = os.getenv("MQTT_BROKER", "localhost")  # Default to localhost for local MQTT broker
    MQTT_PORT = int(os.getenv("MQTT_PORT", "1883"))  # Ensure port is int, default 1883
    MQTT_TOPIC_SUB = os.getenv("MQTT_TOPIC_SUB", f"agent/{DEVICE_ID}/cmd")  # Default topic per device
    MQTT_TOPIC_PUB = os.getenv("MQTT_TOPIC_PUB", f"agent/{DEVICE_ID}/status")  # Default topic per device
