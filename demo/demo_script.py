...
class DemoController:
    ...
    def create_device(self, device_id: int, name: str, version: str = "2.0.0"):
        ...
        data = {"id": device_id, "name": name, "version": version}
        try:
...
def send_heartbeat_periodically(
    controller, device_id, name, version="2.0.0", interval=10
):
    while True:
        try:
            url = f"{controller.base_url}/api/device/heartbeat"
            data = {"name": name, "version": version, "status": "online"}
...
def demo_scenario_1_basic_operation():
    ...
    for i in range(1, 6):
        ...
        threading.Thread(
            ...
            args=(controller, i, f"raspberry-pi-{i:02d}", "2.0.0", 10),
            daemon=True,
        ).start()
